...
执行进度：T001 `in_review`；T002 `in_review`；T003 `in_progress`；T004 `in_progress`；T014 `in_progress`（蒸馏 FP32/PTQ/fake-QAT 为 `75/100`、`77/100`、`78/100`，teacher `95/100`，depthwise 决策 `keep_pointwise`）；T015 `in_progress`（v0.3 RTL/Vivado/package gate 和正确板端 `192.168.68.120` action parity 通过，PR 仍为 Draft，模型和权重不变）。
